...
/**
 * Pixel chip with included leading checked icon as well as text content slot.
 *
        ...
        colors = FilterChipDefaults.filterChipColors(
            ...
            disabledContainerColor = if (selected) {
                MaterialTheme.colorScheme.onBackground.copy(
                    alpha = PixelChipDefaults.DISABLED_CHIP_CONTAINER_ALPHA,
                )
            } else {
                Color.Transparent
            },
...
fun ChipPreview() {
    PixelTheme {
        PixelBackground(modifier = Modifier.size(80.dp, 20.dp)) {
            PixelFilterChip(selected = true, onSelectedChange = {}) {
                Text("Chip")
            }
        }
    }
